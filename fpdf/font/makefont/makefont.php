...
{
	//Read a map file
	$file=dirname(__FILE__).'/'.Toolbox::strtolower($enc).'.map';
	$a=file($file);
	if (empty($a))
...
function MakeFontDescriptor($fm,$symbolic)
{
	...
	if (isset($fm['StdVW']))
		$stemv=$fm['StdVW'];
	else if (isset($fm['Weight']) and eregi('(bold|black)',$fm['Weight']))
		$stemv=120;
	else
		$stemv=70;
	$fd.=",'StemV'=>".$stemv;
...
function MakeFont($fontfile,$afmfile,$enc='cp1252',$patch=array(),$type='TrueType')
{
	...
	if ($fontfile)
	{
		$ext=Toolbox::strtolower(substr($fontfile,-3));
		if ($ext=='ttf')
			$type='TrueType';
		else if ($ext=='pfb')
			$type='Type1';
