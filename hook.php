...
function plugin_manageentities_install()
{
            ...
            if ($DB->tableExists("glpi_plugin_manageentity_profiles") && $DB->fieldExists(
                ...
                if ($DB->tableExists("glpi_plugin_manageentity_config") && !$DB->fieldExists(
                    ...
                    if ($DB->tableExists("glpi_plugin_manageentity_profiles") && !$DB->tableExists(
                        ...
                        if ($DB->tableExists("glpi_plugin_manageentities_profiles") && !$DB->tableExists(
                            ...
                            if ($DB->tableExists("glpi_plugin_manageentities_configs") && !$DB->fieldExists(
    ...
    if ($DB->tableExists("glpi_plugin_manageentities_cridetails") && !$DB->fieldExists(
            "glpi_plugin_manageentities_cridetails",
            "plugin_manageentities_contractdays_id"
        )) {
    ...
    if (!$DB->fieldExists("glpi_plugin_manageentities_configs", "contract_states") && !$DB->tableExists(
            'glpi_plugin_manageentities_business_contacts'
        )) {
...
function plugin_manageentities_uninstall()
{
    ...
    //old versions
    $tables = [
        "glpi_plugin_manageentity_contracts",
...
function plugin_manageentities_addLeftJoin($type, $ref_table, $new_table, $linkfield, &$already_link_tables)
{
    switch ($new_table) {
        case "glpi_plugin_manageentities_criprices" :
            $out = " LEFT JOIN `$new_table` ON (`$ref_table`.`id` = `$new_table`.`plugin_manageentities_critypes_id` AND `$new_table`.`entities_id` IN ('" . implode(
                    "','",
        ...
        case 'Contract' :
            ...
            if (!empty($allPluginContracts)) {
                foreach ($allPluginContracts as $onePluginContract) {
                    ...
                    $pluginContract->update([
                        'id' => $onePluginContract['id'],
            ...
            if (!empty($allPluginContractDays)) {
                foreach ($allPluginContractDays as $onePluginContractDays) {
                    ...
                    $cond = [
                        "`glpi_plugin_manageentities_criprices`.`entities_id`" => $old_entity,
                    ...
                    if (!empty($allPrices)) {
                        foreach ($allPrices as $onePrice) {
                            ...
                            $newPrice = $criPrice->getFromDBbyType(
                                $onePluginContractDays['plugin_manageentities_critypes_id'],
            ...
            if (!empty($allPluginCriDetails)) {
                foreach ($allPluginCriDetails as $onePluginCriDetail) {
                    ...
                    $document->update([
                        'id' => $onePluginCriDetail['documents_id'],
                        'entities_id' => $contract->fields['entities_id']
                    ]);
                    ...
                    $ticket->update([
                        'id' => $onePluginCriDetail['tickets_id'],
                        'entities_id' => $contract->fields['entities_id']
                    ]);

                    $criDetail->update([
                        'id' => $onePluginCriDetail['id'],
                        'contracts_id' => $contract->fields['id'],
                        'entities_id' => $contract->fields['entities_id']
                    ]);
                }
...
function plugin_manageentities_getDatabaseRelations()
{
    if (Plugin::isPluginActive("manageentities")) {
        return [
            ...
            "glpi_users" => [
                "glpi_plugin_manageentities_preferences" => "users_id",
            ...
            "glpi_entities" => [
                "glpi_plugin_manageentities_contracts" => "entities_id",
...
function plugin_manageentities_getDropdown()
{
    if (Plugin::isPluginActive("manageentities")) {
        return [
            'PluginManageentitiesCriType' => __('Intervention type', 'manageentities'),
...
function plugin_manageentities_dynamicReport($parm)
{
    if ($parm["item_type"] == 'PluginManageentitiesFollowUp'
        ...
        if ($parm["item_type"] == 'PluginManageentitiesMonthly'
            && isset($parm["display_type"])) {
