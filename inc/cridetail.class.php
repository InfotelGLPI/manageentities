...
class PluginManageentitiesCriDetail extends CommonDBTM
{
    ...
    function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
    {
        if ($item->getType() == 'Ticket'
            ...
            return self::createTabEntry(PluginManageentitiesCri::getTypeName(1));
        } elseif ($item->getType() == 'PluginManageentitiesContractDay') {
    ...
    static function addReports(Ticket $ticket, $options = [])
    {
        ...
        if (Session::haveRight(
                ...
                UPDATE
            ) && (empty($cridetail) || (isset($cridetail['documents_id']) ? $cridetail['documents_id'] : 0) == 0) && !empty($cridetail['contracts_id']) && !empty($cridetail['plugin_manageentities_contractdays_id'])) {
            $generation_ok = true;
        }
        ...
        if (Session::haveRight(
                "plugin_manageentities_cri_create",
                UPDATE
            ) && (empty($cridetail) || (isset($cridetail['documents_id']) ? $cridetail['documents_id'] : 0) == 0) && (isset($cridetail['withcontract']) ? !$cridetail['withcontract'] : true)) {
            $generation_ok = true;
        }
    ...
    static function showReports($item, $instID, $entity = -1, $condition = [])
    {
        global $DB, $CFG_GLPI;
    ...
    static function setConso($actiontime, $conso, $config, $dataCriDetail, $pluginContract, $numberTask = 0)
    {
        ...
        if ($config->fields['hourorday'] == PluginManageentitiesConfig::DAY) {//configuration by day
            ...
            if ($pluginContract['contract_type'] == PluginManageentitiesContract::CONTRACT_TYPE_INTERVENTION) {
                $conso = $numberTask;
            } elseif ($pluginContract['contract_type'] == PluginManageentitiesContract::CONTRACT_TYPE_HOUR || $pluginContract['contract_type'] == PluginManageentitiesContract::CONTRACT_TYPE_UNLIMITED) {
                $tmp = $actiontime / 3600;
                $conso += $tmp;
            } else {
    ...
    static function showContractLinkDropdown($cridetail, $entities_id, $type = 'ticket')
    {
        ...
        if ($type == 'ticket') {
            ...
            Dropdown::show('PluginManageentitiesContractDay', [
    ...
    static function populatePlanning($options = [])
    {
        ...
        if ($DB->numrows($result) > 0) {
            for ($i = 0; $data = $DB->fetchArray($result); $i++) {
                ...
                $interv[$key]["content"] = Glpi\RichText\RichText::getSafeHtml(
