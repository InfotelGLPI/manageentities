...
//             We get contract days datas
                if ($nbContractDay > 0) {
                    foreach ($iteratord as $dataContractDay) {
                        ...
                        $resultCriDetail_beforeMonth = PluginManageentitiesCriDetail::getCriDetailData(
                            $dataContractDay,
                            [
                                'contract_type_id' => $dataContractDay['contract_type'],
                        ...
                        if (sizeof($resultCriDetail['result']) > 0) {
                            ...
                            $tabResults[$dataEntity['entities_id']][$dataContractDay['contractdays_id']]['contract_type'] = PluginManageentitiesContract::getContractType(
                            ...
                            } elseif ($dataContractDay['contract_type'] == PluginManageentitiesContract::CONTRACT_TYPE_FORFAIT) {
    ...
    static function showMonthly($values = [])
    {
        ...
        if (count($iterator) > 0 && $output_type == search::HTML_OUTPUT) {
            echo "<div class = 'alert alert-important alert-warning d-flex'>" . __(
        ...
        $message_header .= Search::showHeaderItem(
            $output_type,
                        ...
                        foreach ($dataContractDay as $dataTask) {
                            if (is_array($dataTask)) {
                                foreach ($dataTask['conso_per_tech'] as $users_id => $conso) {
                                    ...
                                    $row_num++;
                                    $num = 0;
                                    $message_body .= self::showNewLine(
                                        $output_type,
                                        ($row_num % 2),
                                        $dataContractDay['is_closed'],
                                        $dataContractDay['contractstates_color']
                                    );

                                    // Client
                                    if ($firstEntity) {
                                        $message_body .= Search::showItem(
                                            $output_type,
                                            $dataEntity['entities_name'],
                                            $num,
                                            $row_num,
                                            $depassClass
                                        );
                                    ...
                                    $message_body .= Search::showItem(
                                        $output_type,
                                        $dataContractDay['name_contractdays'],
                                        $num,
                                        $row_num,
                                        $depassClass
                                    );
                                    ...
                                    $message_body .= Search::showItem(
                                        $output_type,
                                        Html::formatNumber($dataContractDay['contract_credit'], 0, 2),
                                        $num,
                                        $row_num,
                                        $depassClass
                                    );
                                    ...
                                    $message_body .= Search::showItem(
                                        $output_type,
                                        Html::formatNumber($dataContractDay['contract_remaining'], 0, 2),
                                        $num,
                                        $row_num,
                                        $depassClass
                                    );
                                    ...
                                    if ($config->fields['useprice'] == PluginManageentitiesConfig::PRICE) {
                                        $message_body .= Search::showItem(
                                            $output_type,
                                            Html::formatNumber($dataTask['pricecri'], 0, 2),
                                            $num,
                                            $row_num,
                                            $depassClass
                                        );
                                    ...
                                    $message_body .= Search::showItem(
                                        $output_type,
                                        self::checkValue($conso['conso'], $output_type),
                                        $num,
                                        $row_num,
                                        $depassClass
                                    );
                                    ...
                                    if ($config->fields['useprice'] == PluginManageentitiesConfig::PRICE) {
                                        $message_body .= Search::showItem(
                                            $output_type,
                                            Html::formatNumber($conso['conso_amount'], 0, 2),
                                            $num,
                                            $row_num,
                                            $depassClass
                                        );
                                    ...
                                    if (!empty($conso['depass'])) {
                                        $message_body .= Search::showItem(
                                            $output_type,
                                            self::checkValue($conso['depass'], $output_type),
                                            $num,
                                            $row_num,
                                            $depassClass
                                        );
                                        ...
                                        if ($config->fields['useprice'] == PluginManageentitiesConfig::PRICE) {
                                            $message_body .= Search::showItem(
                                                $output_type,
                                                Html::formatNumber($conso['depass_amount'], 0, 2),
                                                $num,
                                                $row_num,
                                                $depassClass
                                            );
                                    ...
                                    $message_body .= Search::showItem(
                                        $output_type,
                                        $dataContractDay['contractdays_state'],
                                        $num,
                                        $row_num,
                                        $depassClass
                                    );
        ...
        if ($count_tasks) {
            ...
            $message_body .= Search::showHeaderItem(
                ...
                0,
                '',
                "style='" . PluginManageentitiesMonthly::$style[1] . "'"
            );
            ...
            if ($results) {
                ...
                $message_body .= Search::showFooter(
                    $output_type,
                    __('Entities portal', 'manageentities') . " - " . __('Monthly follow-up', 'manageentities')
                );
            }
            if ($output_type == Search::HTML_OUTPUT) {
                PluginManageentitiesFollowUp::printPager(
                    $start,
                    $numrows,
                    $_SERVER['PHP_SELF'],
                    $parameters,
                    "PluginManageentitiesMonthly"
                );
            }
