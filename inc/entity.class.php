...
//         }
            ...
            if ((Session::getCurrentInterface() == 'central')
                ...
                if ($PluginManageentitiesCri->canView()) {
                    $tabs[7] = PluginManageentitiesCriDetail::createTabEntry(
...
//               $PluginManageentitiesEntity->showTickets($entities);
                ...
                case 7:
                    ...
                    if ((Session::getCurrentInterface() == 'central')
                        || (Session::getCurrentInterface() == 'helpdesk'
                            && $config->fields['choice_intervention'] == PluginManageentitiesConfig::REPORT_INTERVENTION)) {
                        $PluginManageentitiesCriDetail->showReports(
                            0,
                            0,
                            $entities,
        ...
        foreach ($entities as $instID) {
            ...
            if ($logo->getFromDBbyCrit(['entities_id' => $entity->fields["id"]])) {
                echo "<img height='50px' alt=\"" . __s(
                        'Picture'
                    ) . "\" src='" . $CFG_GLPI["root_doc"] . "/front/document.send.php?docid=" . $logo->fields["logos_id"] . "'>";
            }
            echo "</td>";
