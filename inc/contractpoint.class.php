...
//      }
	...
	//Toolbox::logInFile('contractPointCron',"contrat id :".$data['contracts_id']."\n");
        ...
        ".$entity->getFriendlyName()."<br>
         ...
         $sumpoints = 0;
//	Toolbox::logInFile('contractPointCron', 'cri details '.print_r($cridetails,true)."\n");
...
//            $html .= "$count";
	...
	&& $data['contract_type'] == self::CONTRACT_POINTS
) {
            $rest;
...
//         $pdf->Output($entity->getFriendlyName()."-".$month[$month_number].$year.".pdf", 'D');
	 ...
	 //Toolbox::logInFile('contractPointCron', $user_email."\n");
...
//               Toolbox::logInFile('mail_manageentities',__('Error sending email with document','manageentities'). "<br/>" . $mmail->ErrorInfo,true);
            ...
            Session::addMessageAfterRedirect(__('Error sending email with document : No email','manageentities')." ".$data['contracts_id'], false,ERROR);
